...
export const MapViewer = props => {
  ...
  useEffect(() => {
    ...
    // must make each PUMA entry in _PUMAS like this:
    console.log(props);
    props.Counties.forEach(({ county, coordinates }) => {
      let long_lat = coordinates[0].split(" ");
      let longitude = parseInt(long_lat[0], 10);
      let latitude = parseInt(long_lat[1], 10);

      console.log(long_lat, latitude, longitude);
      let location = {
        type: "Feature",
        properties: {
          name: county,
      ...
      L.geoJson(location).addTo(_map.current);
    });
