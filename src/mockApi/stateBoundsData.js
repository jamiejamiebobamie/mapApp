...
const findMin = (num1, num2) => Math.min(num1, num2);
const findMax = (num1, num2) => Math.max(num1, num2);

const getDataFromMultiPolygon = (coords, minLong, minLat, maxLong, maxLat) => {
  coords.forEach(coordBlock => {
    coordBlock[0].forEach(coord => {
      if (!minLong || !minLat || !maxLong || !maxLat) {
        ...
        maxLong = findMax(maxLong, coord[0]);
        maxLat = findMax(maxLat, coord[1]);
        minLong = findMin(minLong, coord[0]);
        minLat = findMin(minLat, coord[1]);
      }
    });
  });
...
const getDataFromPolygon = (coords, minLong, minLat, maxLong, maxLat) => {
...
let data = [];
// parses the 'statesData' above and creates a minCoord array
// of [minimum_longitude, minimum_latitude] and a maxCoord array
// of [maximum_longitude, maximum_latitude] for each state, which represent
// two points that create a bounding box for each state.
// to test if the current map view overlaps a given state.
let count = 0;
statesData.features.forEach(feature => {
  let state = {
    name: feature.properties.name,
    pplPerSqMile: feature.properties.density,
    min: undefined,
    max: undefined
  };
  ...
  if (feature.geometry.type === "MultiPolygon") {
    [minLong, minLat, maxLong, maxLat] = getDataFromMultiPolygon(
    ...
    [minLong, minLat, maxLong, maxLat] = getDataFromPolygon(
      feature.geometry.coordinates,
      minLong,
      minLat,
      maxLong,
  ...
  state.min = [minLong, minLat]; // southwest corner, leaflet wants coords to be
  // [longitude, latitude] and not the more traditional; [latitude, longitude]
  state.max = [maxLong, maxLat]; // northeast corner, leaflet wants coords to be
  // [longitude, latitude] and not the more traditional; [latitude, longitude]
  state.ID = datausa[count].ID_State;
  state.pop = datausa[count].Population;
  // console.log(state.name, datausa[count].State, state.ID);
